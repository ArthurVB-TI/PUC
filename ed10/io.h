...
static bool IO_rand_reset = false;         
...
static inline int IO_numImpar( int valor )
{
    int resultado = 1;
    for(int i = 1; i < valor; i = i + 1){ resultado = resultado * ((valor * 2) - 1); }
    return resultado;
}

static inline int IO_random( int min, int max )
{
    int resultado = 0;
    int random = 0;
    if(!IO_rand_reset){
...
static inline void IO_lerMatriz(int *i, int *j){
    bool v = false;
    ...
    do{
        ...
        v = scanf("%d",i); getchar( );
        if(!v){
            IO_printf("Digite um numero inteiro!\n");
        } else {
            if(*i <= 0){
...
static inline bool IO_procurar(char* linha,char* palavra){
    bool retorno = false;
    int index = 0;
    int linha_n = 0;
    int palavra_n = strlen(palavra) - 1;

    for(int i = 0; i < linha_n - palavra_n;i = i + 1){
        while(linha[index] == palavra[index] && palavra[index] != '\0'){
            index = index + 1;
        }
        if(index == palavra_n){
            retorno = true;
...
static inline double** IO_fIndexMatriz_Double(char* arquivo_name,int *i, int *j){
    // Retorno
    double** retorno = (double**) malloc(sizeof(double*) * STR_SIZE);
    int valores[STR_SIZE]; 
    int decimais[STR_SIZE];
    int index = 0;
    int index_V = 0;
    int index_D = 0;
    int coluna = 0;
    
    // Variaveis para mecher com Arquivo
    ...
    // Pegar Matriz no arquivo
    while(!feof(arquivo) && fgets(linha,Tamanho,arquivo)){
        n = strlen(linha) - 1;
        if(PalavraEncontrada){
            retorno[index] = (double*) malloc(sizeof(double) * STR_SIZE);
            index = index + 1;
            for(int i = 0; i < n; i = i + 1){
                if(Armazenando){
                    if(linha[i] >= '0' && linha[i] <= '9'){
                        if(!Decimal){
                            valores[index_V] = (int) linha[i] - 48;
                            index_V = index_V + 1;
                        } else {
                            decimais[index_D] = (int) linha[i] - 48;
                            index_D = index_D + 1;
                        }
                    } else {
                        if((linha[i] == '.' || linha[i] == ',') && !Decimal){
                            Decimal = true;
                        } else {
                            for(int k = 0; k < index_V; k = k + 1){
                                retorno[i][coluna] = (double) (retorno[i][coluna] * 10 + valores[k]);
                            }
                            for(int k = 0; k < index_D; k = k + 1){
                                retorno[i][coluna] = retorno[i][coluna] + (double) decimais[k] / (double)pow((1.0/10.0),(double)k);
                            }
                            coluna = coluna + 1;
                            Armazenando = false;
                        }
            ...
            if(linha,PalavraFinal){PalavraEncontrada = false;}
        }
...
static inline void IO_indexMatriz(int linha, int coluna, double **matriz){
...
static inline void IO_lerArquivo_int(int *n, int arranjo[], char *arquivo_nome){
